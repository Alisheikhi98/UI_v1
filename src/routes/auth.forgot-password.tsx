...
export const Route = createFileRoute("/auth/forgot-password")({
  head: () => ({ meta: [{ title: withAppName("فراموشی رمز عبور") }] }),
...
function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    toast.success("ایمیل ارسال شد!", {
      description: "صندوق ورودی خود را برای دستورالعمل بازنشانی رمز بررسی کنید.",
    });
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md shadow-lg">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              دستورالعمل بازنشانی رمز به <strong>{email}</strong> ارسال شد
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground text-center">
              ایمیل دریافت نکردید؟ پوشه اسپم را بررسی کنید یا با ایمیل دیگری تلاش کنید.
            </p>
          </CardContent>
          <CardFooter className="flex flex-col space-y-4">
            <Button variant="outline" className="w-full" onClick={() => setIsSubmitted(false)}>
              با ایمیل دیگری تلاش کنید
            </Button>
